...
public class CarouselPagerAdapter extends FragmentPagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public CarouselPagerAdapter(ServiceActivity context, FragmentManager fm) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        try {
            if (position == ServiceActivity.FIRST_PAGE)
                scale = BIG_SCALE;
            else
                scale = SMALL_SCALE;

            position = position % ServiceActivity.count;
    ...
    public int getCount() {
        int count = 0;
        try {
            count = ServiceActivity.count * ServiceActivity.LOOPS;
        } catch (Exception e) {
